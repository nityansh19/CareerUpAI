...
function IntelligenceCore() {
  return (
    <div className="relative mx-auto h-[470px] w-full max-w-[560px] select-none sm:h-[540px]">
      <div className="core-halo absolute left-1/2 top-1/2 h-[370px] w-[370px] -translate-x-1/2 -translate-y-1/2 rounded-full" />
      <div className="orbit orbit-a absolute left-1/2 top-1/2 h-[330px] w-[330px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10" />
      <div className="orbit orbit-b absolute left-1/2 top-1/2 h-[430px] w-[430px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/[0.06]" />

      <div className="core-float absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
        <div className="core-shell flex h-44 w-44 items-center justify-center rounded-[42px] border border-white/15 bg-white/[0.06] shadow-2xl backdrop-blur-2xl sm:h-52 sm:w-52">
      ...
      <div className="float-card float-card-a absolute left-0 top-[16%] z-30 w-[190px] rounded-2xl border border-white/10 bg-[#0b0f19]/80 p-4 shadow-2xl backdrop-blur-xl sm:left-[-10px]">
        ...
        <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-[91%] rounded-full bg-gradient-to-r from-[#7867ff] to-[#d9b45a]" /></div>
        <p className="mt-2 text-right text-[10px] text-white/35">91% match</p>
      </div>

      <div className="float-card float-card-b absolute right-0 top-[20%] z-30 w-[170px] rounded-2xl border border-white/10 bg-[#0b0f19]/80 p-4 shadow-2xl backdrop-blur-xl sm:right-[-4px]">
      ...
      <div className="float-card float-card-c absolute bottom-[12%] left-[6%] z-30 w-[178px] rounded-2xl border border-white/10 bg-[#0b0f19]/80 p-4 shadow-2xl backdrop-blur-xl">
      ...
      <div className="float-card float-card-d absolute bottom-[7%] right-[5%] z-30 w-[188px] rounded-2xl border border-white/10 bg-[#0b0f19]/80 p-4 shadow-2xl backdrop-blur-xl">
...
export default function Home3D() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#060811] text-white">
      <style>{`
        ...
        .hero-grid{background-image:linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px);background-size:72px 72px;mask-image:linear-gradient(to bottom,black,transparent 82%)}
        .nav-glow{box-shadow:0 18px 70px rgba(0,0,0,.32)}
        .core-halo{background:radial-gradient(circle,rgba(120,103,255,.24) 0%,rgba(217,180,90,.12) 34%,transparent 70%);filter:blur(20px);animation:halo 5s ease-in-out infinite}
        .core-shell{transform:rotateX(7deg) rotateY(-9deg);box-shadow:0 40px 90px rgba(0,0,0,.5),inset 0 1px 0 rgba(255,255,255,.12)}
        .core-float{animation:coreFloat 6s ease-in-out infinite}
        .orbit::after{content:"";position:absolute;top:-5px;left:50%;width:9px;height:9px;border-radius:999px;background:#d9b45a;box-shadow:0 0 22px rgba(217,180,90,.8)}
        .orbit-a{animation:spin 18s linear infinite}
        .orbit-b{animation:spinReverse 26s linear infinite}
        .float-card{box-shadow:0 20px 70px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.04)}
        ...
        @keyframes halo{50%{transform:translate(-50%,-50%) scale(1.09);opacity:.8}}
        @keyframes coreFloat{50%{transform:translate(-50%,-53%) rotate(1deg)}}
        @keyframes spin{to{transform:translate(-50%,-50%) rotate(360deg)}}@keyframes spinReverse{to{transform:translate(-50%,-50%) rotate(-360deg)}}
        @keyframes cardA{50%{transform:translate3d(8px,-12px,0) rotate(-1deg)}}@keyframes cardB{50%{transform:translate3d(-8px,10px,0) rotate(1deg)}}@keyframes cardC{50%{transform:translate3d(9px,9px,0) rotate(1deg)}}@keyframes cardD{50%{transform:translate3d(-9px,-10px,0) rotate(-1deg)}}
        @media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms!important;animation-iteration-count:1!important;scroll-behavior:auto!important}}
      `}</style>

      <div className="pointer-events-none fixed inset-0 z-0">
        <div className="hero-grid absolute inset-0" />
        <div className="absolute left-[-12%] top-[-18%] h-[600px] w-[600px] rounded-full bg-[#7867ff]/10 blur-[150px]" />
        <div className="absolute right-[-12%] top-[20%] h-[520px] w-[520px] rounded-full bg-[#d9b45a]/[0.07] blur-[150px]" />
      ...
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "border-b border-white/[0.07] bg-[#060811]/80 backdrop-blur-2xl nav-glow" : "bg-transparent"}`}>
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-6 lg:px-10">
          <Link to="/" className="group flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-[13px] border border-[#d9b45a]/30 bg-[#d9b45a]/10 font-semibold text-[#efd080] shadow-[0_0_30px_rgba(217,180,90,.08)] transition group-hover:rotate-6">C</div>
          ...
          <nav className="hidden items-center gap-8 md:flex">
            ...
            <Link to="/register" className="rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-[#090b12] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_35px_rgba(255,255,255,.12)]">Get started</Link>
          </nav>
      ...
      <main className="relative z-10">
        <section className="mx-auto grid min-h-[92vh] max-w-7xl items-center gap-10 px-6 pb-16 pt-32 lg:grid-cols-[1.02fr_.98fr] lg:px-10 lg:pb-20 lg:pt-28">
          <div className="relative z-10 max-w-3xl">
            ...
            <div className="reveal reveal-3 mt-9 flex flex-wrap items-center gap-3"><Link to="/register" className="group inline-flex items-center gap-2 rounded-2xl bg-white px-6 py-3.5 text-sm font-semibold text-[#080a11] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(255,255,255,.13)]">Build my career profile <span className="transition group-hover:translate-x-1"><Arrow /></span></Link><a href="#intelligence" className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.025] px-6 py-3.5 text-sm font-medium text-white/65 backdrop-blur-xl transition hover:bg-white/[0.06] hover:text-white">See how it works</a></div>
          ...
          <div className="relative"><IntelligenceCore /></div>
        </section>
        ...
        <section id="product" className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-28">
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-3">
            {featureItems.map((item, index) => <TiltCard key={item.title} className="shine-border rounded-[28px] border border-white/[0.07] bg-white/[0.025] p-6 transition-transform duration-200 will-change-transform"><div className="flex items-center justify-between"><span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/[0.07] bg-white/[0.035] text-[#efd080]">{item.icon}</span><span className="text-[10px] tracking-[.2em] text-white/20">0{index + 1}</span></div><h3 className="mt-9 text-xl font-semibold tracking-[-.025em]">{item.title}</h3><p className="mt-3 text-sm leading-6 text-white/35">{item.text}</p><div className="mt-8 flex items-center gap-2 text-xs font-medium text-white/40">Explore <Arrow size={15} /></div></TiltCard>)}
          </div>
        </section>

        <section id="intelligence" className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32">
          <div className="shine-border overflow-hidden rounded-[36px] border border-white/[0.08] bg-[#0a0d16]/80 p-6 shadow-[0_40px_120px_rgba(0,0,0,.35)] backdrop-blur-xl sm:p-10 lg:p-12">
            <div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
              ...
              <div className="relative min-h-[420px] rounded-[30px] border border-white/[0.07] bg-black/20 p-5 sm:p-7">
                ...
                <div className="relative mt-8 grid gap-3 sm:grid-cols-2">{[["91%","Career fit"],["84","Readiness score"],["4","Priority gaps"],["6","Next actions"]].map(([value,label]) => <div key={label} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5"><p className="text-3xl font-semibold tracking-[-.05em]">{value}</p><p className="mt-2 text-[10px] uppercase tracking-[.16em] text-white/25">{label}</p></div>)}</div>
                <div className="relative mt-3 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#7867ff]/10 text-[#9f94ff]"><Spark /></span><div><p className="text-xs font-semibold">Highest-value next move</p><p className="mt-1 text-xs text-white/30">Strengthen system design and ship one architecture-focused project.</p></div></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-[76%] rounded-full bg-gradient-to-r from-[#7867ff] to-[#d9b45a]" /></div></div>
              </div>
            </div>
          </div>
        </section>
